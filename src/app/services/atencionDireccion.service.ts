...
@Injectable()
export class AtencionDireccionService {
    public url: string;
    public identity;
    public token;
    ...
    atenderRequerimiento(token, atencionDireccion): Observable<any> {

        let json = JSON.stringify(atencionDireccion);
        let params = 'json=' + json;
        let headers = new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded')
                                       .set('Authorization', token);
        return this._http.post(this.url + 'ingresar-requerimiento-direccion', params, { headers: headers });


    }

    obtenerAtencionDireccion(token, id_requerimiento){
       ...
       //let name = id_requerimiento;
          ...
          return this._http.post(this.url + 'atencion-direccion', name, { headers: headers });
