...
@Injectable()
export class AtencionAreaService {
    public url: string;
    public identity;
    public token;
    ...
    atenderRequerimiento(token, atencionArea): Observable<any> {

        let json = JSON.stringify(atencionArea);
        let params = 'json=' + json;
        let headers = new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded')
                                       .set('Authorization', token);
        return this._http.post(this.url + 'ingresar-requerimiento-area', params, { headers: headers });



    }

    obtenerAtencionArea(token, id_requerimiento){
       ...
       //let name = id_requerimiento;
          ...
          return this._http.post(this.url + 'atencion-area', name, { headers: headers });
