...
@Injectable()
export class RespuestaDireccionService {
    public url: string;
    public identity;
    public token;
    ...
    responderRequerimiento(token, respuestaDireccion, idRequerimiento): Observable<any> {

        let fecha = Date.now();
        moment.locale('es');
        let formatFecha = moment(fecha).format('YYYY-MM-DD HH:mm:ss');

        let json = JSON.stringify(respuestaDireccion);
        let params = 'json=' + json;
        let headers = new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded')
            .set('Authorization', token);
        return this._http.post(this.url + 'respuesta-requerimiento-direccion/' + idRequerimiento + '/' + formatFecha, params, { headers: headers });
    ...
    obtenerRespuestaDireccion(token, idResDepa) {
        let name;

        let headers = new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded')
            .set('Authorization', token);

        return this._http.post(this.url + 'obtener-respuesta-direccion/' + idResDepa, name, { headers: headers });
